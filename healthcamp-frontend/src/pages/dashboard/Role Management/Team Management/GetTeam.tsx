...
import { getTeamById, removemember } from "../../../../api/team";
...
const GetTeam = () => {
  ...
  const { isLoading, data } = useQuery({
    ...
    queryFn: async () => {
      {
        const response = await axiosPrivateInstance.get(`${getTeamById}/${id}`);
  ...
  const { isPending, mutate: removeMutate } = useMutation({
    ...
    <>
      ...
      <Paper p={40} withBorder>
        <Flex direction="column" align="center" gap={10}>
          ...
          <Image mt={20} w={150} radius="50%" src={data?.teamLeader?.profile} />
        ...
        <Tabs mt={30} variant="pills" defaultValue="team">
          ...
          <Tabs.Panel value="subteam">
            {serviceData?.servicesWithCalendarStatus?.map(
              (item1: any, index1: any) => {
                return (
                  <Group key={index1} gap={0} mt={10} justify="space-between">
                    <Paper w={isMobile ? "80%" : "92%"} p={20} bg="blue">
                      ...
                      <IoIosAddCircle
                        onClick={() => {
                          navigate(`/create-subteam`, {
                            state: { teamId: id, serviceId: item1.id },
                          });
                        }}
                        size={30}
                        color="#6092FE"
